...
contract TokenCreateContract is HederaTokenService, ExpiryHelper, KeyHelper {
    ...
    function createFungibleTokenPublic() public payable returns(address) {
        ...
        theTOKEN = tokenAddress;
    }


    function transferFT() public {
        // require(balanceOf(msg.sender, token) = 0);
        int64 amount = 100;
        // address receiver = msg.sender;

        // (int responseCode1) =
        HederaTokenService.associateToken(address(msg.sender), theTOKEN);

        (int responseCode2) =
        HederaTokenService.transferToken(theTOKEN, address(this), address(msg.sender), amount);
